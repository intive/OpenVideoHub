                    ...
                    int stride, int x, int y)
{
    ...
    int mask = (1 << s->vp56_coord_div[b]) - 1;
    int deblock_filtering = 0; //s->deblock_filtering;
    int dx;
    int dy;
...
#if 1
    dx = (s->mv[b].x >> s->vp56_coord_div[b]) + ( (s->mv[b].x >> 31) && (s->mv[b].x & mask) );
    dy = (s->mv[b].y >> s->vp56_coord_div[b]) + ( (s->mv[b].y >> 31) && (s->mv[b].y & mask) );
#else
    // signed division by shifting; for negative input with non-zero remainder the quotient must be incremented
    {
        register int tmp;
        asm volatile (
                "mov    %0, %3, asr %5        \n\t"
                ...
                "addne    %1, %1, %4, lsr #31"
                : "=r" (dx), "=r" (dy), "=r" (tmp)
                : "r" (s->mv[b].x), "r" (s->mv[b].y), "r" (s->vp56_coord_div[b]), "r" (mask)
                );
    }
#endif
