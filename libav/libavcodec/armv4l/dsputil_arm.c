...
extern void clear_block_arm(DCTELEM *data);
...
static void simple_idct_armv5te_put(uint8_t *dest, int line_size, DCTELEM *block)
{
    simple_idct_put_armv5te (dest, line_size, block);
    clear_block_arm( block );
}
static void simple_idct_armv5te_add(uint8_t *dest, int line_size, DCTELEM *block)
{
    simple_idct_add_armv5te( dest, line_size, block );
    clear_block_arm( block );
}
...
static void clear_blocks_arm( DCTELEM *blocks )
{
}
...
void dsputil_init_armv4l(DSPContext* c, AVCodecContext *avctx)
{
    ...
    if( idct_algo != FF_IDCT_VP3 )
    {
        c->idct_put= simple_idct_armv5te_put;
        c->idct_add= simple_idct_armv5te_add;
        c->idct    = simple_idct_armv5te;
        c->idct_permutation_type = FF_NO_IDCT_PERM;
    }
    c->clear_blocks = clear_blocks_arm;
