...
HBufC* CPlugin::ConvertUtfToUnicodeL( const char *aUtf7 )
	{
	TInt len = strlen( aUtf7 );
	TPtr8 string((TUint8 *)aUtf7, len, len);

	HBufC8* tmp = string.AllocL();
				...
				if( media )
					...
					if( media_title )
						...
						if( t )
							...
							HBufC* mediaTitle = ConvertUtfToUnicodeL( t );
					...
					if( media_player )
						...
						if( url )
							...
							TInt index = u->LocateReverse( '=' );
							TPtrC id = u->Right( u->Length() - index - 1 );
...
_LIT8( KTParam, "\"t\":" );
HBufC* CPlugin::VideoUrlL( const TDesC8& aResponse )
	{
	RBuf newUrl;
...
//login
_LIT( KYouTubeLoginUrl, "http://youtube.com/login" );
...
void CPlugin::LoginUrlAndBodyL( HBufC*& aUrl, HBufC8*& aBody, HBufC8*& aContentType, const TDesC& aUserName, const TDesC& aPassword )
	...
	HBufC8* body = HBufC8::NewL( KYoTubeLoginData().Length() + aUserName.Length() + aPassword.Length() + KYouTubeUsername().Length() + KYouTubePassword().Length() );
...
//upload video
_LIT( KYouTubeUploadVideoUrl, "http://youtube.com/my_videos_upload" );
...
TInt CPlugin::ParseUploadMovieResponseStep2L( const TDesC8& aResponse )
	{
	TInt offset = aResponse.Find( _L8("Thank you! Your upload is complete.") );
	if( offset == KErrNotFound )
		{
		return KErrNotFound;
		}
